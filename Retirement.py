...
# ──────────────────────────────────────────────
    ...
    st.title("🗒️ 更新紀錄")
    st.caption("本頁記錄近期針對 UX、顯示口徑與風險剖析所做的改動，方便回溯與驗證。")

    st.subheader("2026-04｜口徑與顯示一致性")
    st.markdown("""
- **實質/名目口徑全站統一**：核心計算以 **2026 實質購買力**為主；名目金額只作顯示換算（以 CPI 複利估算）。
- **核心結果置頂**：在「📊 退休規劃」頁最上方新增框選指標：
  - 退休成功率（標準蒙地卡羅）
  - 目標年齡剩餘資產：**名目金額**為主、並顯示「折算回 2026」作為購買力對照
  - 固定提領「剛好歸零」的臨界 IWR（確定性上限）
- **壓力測試表格口徑對齊**：各格統一顯示「**名目剩餘資產（括號=折算回2026：實質購買力）**」。
""")

    st.subheader("2026-04｜失敗路徑剖析（蒙地卡羅）")
    st.markdown("""
- **新增失敗路徑剖析**：當成功率非 100% 時，可分析「哪些路徑在幾歲歸零」。
- **歸零年齡直方圖**：視覺化失敗集中區間（SORR/長壽風險的落點）。
- **最差 N 條資產曲線**：以「實質購買力」呈現最壞路徑的資產軌跡。
- **CSV 下載**：提供每條路徑的摘要（含 `success/ruin_age/final_real/final_nominal/min_real/early_return` 等欄位），便於外部分析。
""")

    st.subheader("2026-04｜不動產欄位 UX（不改引擎）")
    st.markdown("""
- **租金淨收入說明強化**：明確定義為「扣掉空置、修繕、管理、稅費後的淨現金流」，並提供保守估算提示（例如毛租金×0.75）。
- **流動性折扣文案校正**：強調其意義為交易成本/稅費等「變現摩擦」，不是市場價格下跌假設。
- **以房養老定位調整**：以「晚年流動性保險/長照備援」視角提醒使用情境。
""")

    st.subheader("備註")
    st.info(
        "本頁為開發紀錄摘要。\n\n"
        "- 若你需要更像產品的版本（例如：每條改動附上影響範圍、對應頁面、或截圖指引），我可以再把紀錄結構化。\n"
        "- 若你想把這份紀錄也輸出成檔案（Markdown/CSV），我也可以加一個下載按鈕。"
    )
