# -*- coding: utf-8 -*-
"""
ASCII 檔名版本（Streamlit Cloud / Linux 環境較穩定）
內容與 `01_教育資訊庫.py` 同步，避免非 ASCII 檔名在部署環境造成解析/載入問題。
"""
...
- ETF 僅是工具：需搭配「提領策略（固定/微笑/GK）」與風險承受度。
    ...
    st.warning("本區塊目前僅作教育整理；若要升級為『官方級資訊庫』，建議改為引用 IMF/主計總處/央行/政府公開統計，並在內文標示來源與更新日期。")

elif topic.startswith("9"):
    st.subheader("退休金制度改革動態（2026 觀察）")
    st.warning("本區塊目前尚未做成可更新的官方來源彙整（例如：主管機關公告、法規修正、精算報告）。若要『官方級』，建議改為結構化欄位（來源機關/文件名/發布日/摘要/連結），並定期更新。")
